...
new_df_Reeng["Impressions"] = df_DBM_Reeng.groupby(["Date"]).sum()['Impressions']
new_df_Reeng["Clicks"] = df_DBM_Reeng.groupby(["Date"]).sum()['Clicks']
new_df_Reeng["Revenue (Adv Currency)"] = df_DBM_Reeng.groupby(["Date"]).sum()['Revenue (Adv Currency)']
...
if new_df_android.index[-1] == new_df_Reeng.index[-1] and new_df_android.index[-1] == new_df_IOS.index[-1]:
...
elif new_df_android.index[-1] == new_df_IOS.index[-1] and new_df_android.index[-1] != new_df_Reeng.index[-1]:
...
elif new_df_android.index[-1] == new_df_Reeng.index[-1] and new_df_android.index[-1] != new_df_IOS.index[-1]:
    df_Total["Registrations"] = new_df_android["Registrations"] + new_df_Reeng["Registrations"] 
    df_Total["Activations"] = new_df_android["Activations"] + new_df_Reeng["Activations"] 
...
creative_Reeng = pd.merge(df_DBM_Reeng, df_af_Reeng_use, left_on = ["CM Placement ID", "Date"], right_on = ["CM Placement ID", "Date"]
         ,how = 'outer').groupby(['Insertion Order', 'Line Item', "Creative"]).sum()
...
print(message)
my_speech(message)import pandas as pd
import os
import numpy as np
from datetime import datetime, timedelta
from stat import S_ISREG, ST_CTIME, ST_MODE
